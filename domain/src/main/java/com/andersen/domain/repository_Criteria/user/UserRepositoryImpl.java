package com.andersen.domain.repository_Criteria.user;
...
public class UserRepositoryImpl implements UserRepository {
    ...
    public User createUser(User user) throws DataAccessException {
        try {
            ...
            throw new DataAccessException("Failed to create user", ErrorCodeRepo.US_002);
        }
    }
    ...
    public Optional<User> getUserById(Long id) throws DataAccessException {
        try {
            ...
            throw new DataAccessException("Failed to get user by id", ErrorCodeRepo.BK_005);
        }
    }
    ...
    public Optional<User> getUserByUsername(String username) throws DataAccessException {
        try {
            ...
            throw new DataAccessException("Failed to get user by username", ErrorCodeRepo.US_007);
        }
    }
    ...
    public boolean updateUser(User user) throws DataAccessException {
        try {
            if (user == null || user.getId() == null) {
                throw new DataAccessException("Invalid user data for update", ErrorCodeRepo.US_003);
            }

            // Get the existing user without merging
            User existing = entityManager.find(User.class, user.getId());
            if (existing == null) {
                throw new DataAccessException("User not found", ErrorCodeRepo.US_007);
            }
            ...
            throw new DataAccessException("Failed to update user", ErrorCodeRepo.US_003);
        }
    }
    ...
    public boolean deleteUser(Long id) throws DataAccessException {
        try {
            ...
            throw new DataAccessException("Failed to delete user", ErrorCodeRepo.US_004);
        }
    }
}