package com.andersen.repository_Criteria.workspace;
...
public class WorkspaceRepositoryImpl implements WorkspaceRepository {
    ...
    public List<Workspace> getAvailableWorkspaces(LocalDateTime startTime, LocalDateTime endTime) throws DataAccessException {
        try {
            ...
            bookingSubquery.select(bookingRoot.get("workspace").get("id"))
                    .where(
                            cb.equal(bookingRoot.get("status"), BookingStatus.CONFIRMED),
                            cb.lessThan(bookingRoot.get("startTime"), endTime),
                            cb.greaterThan(bookingRoot.get("endTime"), startTime)
                    );

            // Main query: active workspaces NOT in the booking subquery
            query.where(
                    cb.isTrue(workspaceRoot.get("isActive")),
                    cb.not(workspaceRoot.get("id").in(bookingSubquery))
            );
